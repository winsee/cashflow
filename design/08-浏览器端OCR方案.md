...
- **串行**：一帧没识别完不抓下一帧。认出候选但还没锁定时立刻扫下一帧（要尽快凑够
  「连续一致」），走服务端识别时保留原来的 1.2s 保护间隔
- **识别稳了就锁定**（见 §3.5）：候选区不再被后续帧覆盖，画面定格，玩家从容点选
...
连续扫描原本一直跑，每一帧都无条件重写候选区——包括写成空数组（`no_match` 时候选当场
消失）。玩家举着卡准备点确认，摄像头一晃候选就跳没了，可点窗口只有一帧的时间。
改成**扫到稳定候选就停扫**：

- **锁定判据**（`CardPicker.vue` 的 `shouldLock`）：top1 分数 ≥ `LOCK_SCORE 0.85`
  **且**甩开第二名 ≥ `LOCK_MARGIN 0.10` → 当帧就锁；分数一般 → 要连续
  `STABLE_FRAMES 2` 帧 top1 是同一张卡才锁。**不能只看分数**——刚举起卡那几帧往往
  还没对准，一张糊帧就能把玩家锁死在错卡上
- **阈值有实测支撑**：194 张实拍的离线文本上，top1 认错的 20 例 margin **全是 0**
  （同标题不同版本并列打平，§3.4 那两个 bug 修完后剩下的就是这类），所以「甩开第二名」
  这条门本身就把误锁挡光了。0.85/0.10 下 60% 的正确卡当帧快锁、误锁 0 例
- **空结果不覆盖已有候选**（`applyCandidates`）：手一抖糊掉一帧不该把候选区整块抹掉。
  `lastRecognitionId` 一并保留，FR-28 的命中统计因此始终对应玩家真正看到的那次识别
- **锁定 = 暂停 `<video>`，不停媒体流**：画面定格给出「停了」的视觉信号，
  「🔄 重新扫描」才能瞬时恢复（无黑屏、不再要摄像头权限）。玩家点选卡片或关窗口时
  才真正 `stopScan()` 释放摄像头
- 熔断（硬失败连续 3 次 / `unavailable` 一次）与手动检索兜底原样不变
...
1. **真机取景帧质量**（最大不确定性，§6.2 还没做）。缓解手段按顺序：已做裁剪扫描框；
   已做"连续两帧同一候选才锁定"（§3.5）；再不够上灰度化/二值化预处理
...
顺带核 §3.5 的锁定手感：卡还没放正时不该被锁死；对准后一两帧内锁住、画面定格；
锁错了点「🔄 重新扫描」应立刻恢复实时取景（无黑屏、不再要权限）。
真机若普遍锁得太慢（分数上不去、总要凑连续两帧），调 `LOCK_SCORE` 比调
`LOCK_MARGIN` 安全——误锁全部发生在 margin=0 的并列样本上。
