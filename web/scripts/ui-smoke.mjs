...
// 开发机是 Windows（Edge 优先），但这个脚本在 Linux 容器/CI 里也要能跑——
// Windows 路径排在前面，找不到才往下走，所以房主那台机器上的行为一个字没变。
const BROWSERS = [
...
// 同上：Windows 的 venv 布局排在前面，Linux 的 bin/ 只是找不到时的退路
const py = [
  join(repo, 'server', '.venv', 'Scripts', 'python.exe'),
  join(repo, 'server', '.venv', 'bin', 'python'),
].find(existsSync)
if (!py) { console.error('缺少 server/.venv'); process.exit(1) }
