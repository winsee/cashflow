...
const BOARD_MAX = 332
const BOARD_MIN = 120
/** 格面文字是 SVG 用户单位 9/320，随板宽等比缩放——300 曾是「渲染出来才 8.4px，
 *  再窄就只剩色块」的保守估计，但真机反馈里棋盘常年卡在 300 以下（正方形棋盘在竖屏里
 *  多半是**宽度**先顶到 stage 边界，纵向反而空出一截没用完），把「写字」这个更有用的
 *  状态挡在了门外。用真实两字格名（结算/慈善/失业…）在 3x DPR 下逐档截图比对，240
 *  （≈6.75px）仍然清楚可辨，200（≈5.6px）开始发糊——240 是留了余量的新下限（design/09 §2.2 v0.21）。
 *  所以「格面文字与名牌退不退场」**是板宽的结果，不是档位的结果**——空间够就写字，
 *  不够才退场，与抽屉停在哪一档无关。**工具带两种情况都不看它**：那三枚钮在任何
 *  档位都钉在原地（design/09 §3.2.1 v0.15）。 */
const BOARD_TEXT_MIN = 240
